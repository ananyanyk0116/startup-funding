...
PLOT_DIR = "plots"
# Handle directory creation - 'plots' exists as a file, need to move it
import shutil
import tempfile
if os.path.isfile(PLOT_DIR):
    temp_name = tempfile.mktemp(prefix='plots_backup_', dir='.')
    shutil.move(PLOT_DIR, temp_name)
    print(f"Moved existing file '{PLOT_DIR}' to '{temp_name}'")
    os.makedirs(PLOT_DIR)
elif os.path.isdir(PLOT_DIR):
